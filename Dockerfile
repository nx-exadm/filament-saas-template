...
# 7. Safe runtime script that skips the installer and boots the supervisor service directly
CMD ["/bin/sh", "-c", "if [ -z \"$APP_KEY\" ]; then echo 'Waiting for Render environment variables...'; sleep 10; exit 1; fi && /usr/bin/supervisord", "-c", "/etc/supervisor/conf.d/supervisord.conf"]
